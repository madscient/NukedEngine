...
内部ヘッダー (`FmEngine.h` / ymfm / Nuked コア等) は不要です。  
...
オーディオストリーム開始前に全て完了させてください（オーディオコールバックスレッドとの競合防止）。
...
> パッチデータは YMEngine (ymfm) 用のものをそのまま流用できる。
